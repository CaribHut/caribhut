...
BOOKING_BLOCK_MINUTES = 120  # 2 timmar mellan sittningar på samma bord


def parse_booking_datetime(date_str: str, time_str: str) -> datetime:
    """
    Parse booking date/time from frontend format:
    date: YYYY-MM-DD
    time: HH:MM
    """
    try:
...
def bookings_conflict(date1: str, time1: str, date2: str, time2: str, block_minutes: int = BOOKING_BLOCK_MINUTES) -> bool:
    """
    Returns True if two bookings overlap within the configured block window.
    Same table cannot be booked again until block_minutes have passed.
    """
    dt1 = parse_booking_datetime(date1, time1)
    dt2 = parse_booking_datetime(date2, time2)
    diff_minutes = abs((dt1 - dt2).total_seconds()) / 60
    return diff_minutes < block_minutes


async def get_booked_table_ids_for_slot(date: str, time: str) -> List[int]:
    """
    Returns all table IDs that are unavailable for the given date/time,
    taking the booking block window into account.
    """
    requested_dt = parse_booking_datetime(date, time)

    # Hämta bara dagens bokningar först för att hålla det enkelt och snabbt.
    # Eftersom sittningarna är inom samma dag i praktiken räcker detta normalt.
...
async def get_booking_availability(date: str, time: str):
    ...
    """
    try:
        parse_booking_datetime(date, time)
        booked_table_ids = await get_booked_table_ids_for_slot(date, time)
        return {"bookedTableIds": booked_table_ids}
...
async def create_booking(input: BookingCreate):
    ...
    try:
        # Validera datum/tid-format tidigt
        parse_booking_datetime(input.date, input.time)

        # Hämta bord som redan är blockerade för detta datum/tid
        booked_table_ids = await get_booked_table_ids_for_slot(input.date, input.time)

        if input.table_id in booked_table_ids:
            raise HTTPException(
                status_code=409,
